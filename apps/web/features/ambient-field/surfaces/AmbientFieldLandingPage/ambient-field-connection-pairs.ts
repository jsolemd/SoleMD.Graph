...
export type AmbientFieldClusterId = "papers" | "entities" | "relations";

export interface AmbientFieldConnectionCluster {
  accentVar: string;
  id: AmbientFieldClusterId;
...
export interface AmbientFieldConnectionPair {
  arch?: number;
  color: string;
  direction?: 1 | -1;
  from: number;
...
// one named hotspot (indices 0/1/2 in `ambientFieldBlobHotspots`) plus a
...
export const ambientFieldConnectionClusters: readonly AmbientFieldConnectionCluster[] = [
  {
    id: "papers",
...
function intra(
  cluster: AmbientFieldConnectionCluster,
  from: number,
  to: number,
  direction: 1 | -1 = 1,
): AmbientFieldConnectionPair {
  return {
...
const papers = ambientFieldConnectionClusters[0]!;
const entities = ambientFieldConnectionClusters[1]!;
const relations = ambientFieldConnectionClusters[2]!;
...
export const ambientFieldConnectionPairs: readonly AmbientFieldConnectionPair[] = [
  intra(papers, 0, 3, 1),
  intra(papers, 0, 8, -1),
  intra(papers, 3, 16, 1),
...
if (process.env.NODE_ENV !== "production") {
  for (const pair of ambientFieldConnectionPairs) {
    if (
      pair.from >= ambientFieldBlobHotspots.length ||
      pair.to >= ambientFieldBlobHotspots.length
    ) {
      console.warn(
        `[ambient-field-connection-pairs] pair ${pair.from}->${pair.to} references a hotspot index that is out of range.`,
      );
    }
  }
}
