...
from app.rag.orchestrator import PostgresTargetCorpusLoader
...
from app.rag.source_locator import (
    RagSourceLocatorEntry,
    SidecarRagSourceLocatorRepository,
    locator_sidecar_path,
)
from app.rag.source_locator_checkpoint import (
    ...
    *,
    ...
    if not skip_bioc:
        ...
        for archive_path in archive_paths:
            ...
            for document_index, (document_id, _) in enumerate(_iter_bioc_documents(archive_path), start=1):
                if document_index <= saved_ordinal:
                    continue
                report.bioc_stage.scanned_documents += 1
                if document_id:
                    pending_rows.append((document_id, document_index, archive_path.name))
    ...
    *,
    ...
    if resolve_target is not None:
        resolved = {
            ...
            for document_id, _, _ in pending_rows
        ...
        resolved = resolver.resolve_document_ids([document_id for document_id, _, _ in pending_rows])

    entries: list[RagSourceLocatorEntry] = []
    for document_id, document_index, archive_name in pending_rows:
