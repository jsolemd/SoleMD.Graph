...
|---|---|---|---|---|---|---|---|---|
| 1 | BufferGeometry extended methods | [16000, 16431] | Attribute setters, tangent/normal computation, indexing, JSON serialization. | `ci` | BufferGeometry base class (Three.js core) | `apps/web/features/ambient-field/renderer/field-shaders.ts` (material + geometry pipeline only — consumed via asset registry `ku`) | small (library) | Extends `Wa` (EventDispatcher, outside slice) |
| 2 | Mesh class (`ui`) | [16443, 16599] | Core WebGL mesh container: material, geometry, morph targets, vertex position retrieval. | `ui` | Three.js Mesh | `apps/web/features/ambient-field/renderer/FieldScene.tsx` (stage renderer instantiates meshes) | small (library) | Extends `pi` (Object3D); uses `ci` |
...
| 6 | InstancedMesh (`Fh`) | [17991, 18200] | Instance-based rendering for repeated geometry. | `Fh` | Three.js InstancedMesh | possibly `apps/web/features/ambient-field/controller/*` (particle instancing) | small (library) | Extends `ui` |
...
| 8 | PlaneGeometry / SphereGeometry | [19331, 19467] | Procedural geometry generators. | `Gh`, `Wh` | Three.js primitive geometries | `apps/web/features/ambient-field/asset/*` (consumed via geometry generator `jo`) | trivial (library) | Extends `ci` |
| 9 | MeshStandardMaterial (`hc`) | [19469, 19500] | Physically-based material setup (color, roughness, metalness, maps). | `hc` | Three.js MeshStandardMaterial | `apps/web/features/ambient-field/renderer/field-shaders.ts` (material registry applies shader uniforms) | small (library) | Extends `Rr` (Material base) |
...
| 11 | FileLoader / ImageLoader / TextureLoader | [20587, 20925] | Resource loaders with fetch, abort, progress, MIME handling. | `ha`, `Qf`, `yo`, `Jf`, `gb`, `M0` | Three.js loaders | `apps/web/features/ambient-field/asset/*` (asset registry uses loaders for GLB/PNG) | small (library) | Extends `as` (Loader base); uses `la` (cache) |
...
| 13 | GLSL Shader Chunks (~200 definitions) | [22029, 23990] | Modular GLSL fragments: alpha hash, AO map, batching, morphing, normals, roughness, shadows, dithering, iridescence. | (string constants) | Three.js ShaderChunk library | `apps/web/features/ambient-field/renderer/field-shaders.ts` (Maze's stream particle shader selectively applies these chunks) | trivial (library) | Pure shader code strings |
