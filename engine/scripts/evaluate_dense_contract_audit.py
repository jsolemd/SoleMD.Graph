"""Run dense retrieval contract and reranker audits over runtime-eval cohorts."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare dense biomedical retrieval spaces over the runtime-eval cohort "
            "and optionally rerank top candidates with MedCPT."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of papers to sample from the runtime population (0 = full population).",
    )
