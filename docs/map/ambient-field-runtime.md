...
- the product runtime should be called the `Ambient Field`
- the code package should live at `apps/web/features/ambient-field/`
...
- `Evidence Layer` should refer to the semantic overlays, focus states,
  hotspots, and graph-linked actions that sit on top of the ambient field
...
The objective is to avoid two separate systems:

- a one-off landing page experience
- a separate module experience

Instead, SoleMD should have one shared field runtime that presents different
scene states across the site.

This runtime is the canonical standard for:

- the landing page
- homepage story chapters
- inline modules
- expanded module surfaces
- guided graph-bridge presentations
...
- every module should inherit the living ambient field by default
...
1. one persistent ambient-field runtime
...
### 0. Living ambient field is the default module standard
...
That means all major modules should inherit the ambient field runtime by
...
Continuity with the knowledge web remains mandatory; ambient-field dominance in
every viewport is not.

### 1. One shared runtime, not separate homepage and module systems

The homepage landing experience should become the first production use of the
same runtime that powers future modules.

The runtime should be reusable across:

- `apps/web/app/page.tsx`
- wiki module content rendered through `apps/web/features/wiki/components/WikiModuleContent.tsx`
- inline module views inside `apps/web/features/wiki/components/WikiPanel.tsx`
- expanded module presentations that reuse the same scene and overlay logic

The homepage is therefore the first implementation of the module standard, not
an exception to it.
...
### 2. Ambient field is real-data-derived, not live-data-driven
...
- a branded ambient-field atmosphere that carries through the site
...
- open paper, entity, or cluster detail from ambient/module overlays
...
- preserve ambient/module mode if graph navigation is not requested or fails
...
The ambient runtime must stay visibly lighter than live graph mode.
...
Without budgets, the ambient runtime will drift toward a worse version of live
Cosmograph.
...
Ambient field encoding must not imply evidentiary strength unless tied to a
defined metric.
...
- homepage: full-page ambient field with guided sections and a graph entry CTA
...
the ambient field runtime at the landing layer, with graph workspace entry as a
...
These remain the live graph workspace and graph-bridge target. The ambient field
...
- No live DuckDB dependency for ambient field scenes.
...
- No module implementation that abandons the living ambient-field model plus
