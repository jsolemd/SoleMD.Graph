...
- the current implementation and rollout notes in
  [`field-implementation.md`](./field-implementation.md)
...
Future Field work should prefer the following sources before generic
...
Future module and field work should follow this retrieval order before
implementation:
...
As of 2026-04-18, the field code in
[`../../apps/web/features/field/`](../../apps/web/features/field/)
...
- [`FieldCanvas.tsx`](../../apps/web/features/field/renderer/FieldCanvas.tsx)
  mounts one fixed R3F canvas with `PerformanceMonitor` and `AdaptiveDpr`.
- [`FieldScene.tsx`](../../apps/web/features/field/renderer/FieldScene.tsx)
  renders the active stage family from shared point sources and a shared shader
  contract.
- [`field-shaders.ts`](../../apps/web/features/field/renderer/field-shaders.ts)
  drives FBM, amplitude deformation, ambient drift, stream conveyor/funnel, and
  perspective-weighted point importance.
- [`point-source-registry.ts`](../../apps/web/features/field/asset/point-source-registry.ts)
  is the active point-source asset registry for `blob`, `stream`, and `pcb`,
  with cache reuse and prewarm.
- [`visual-presets.ts`](../../apps/web/features/field/scene/visual-presets.ts)
  defines the current stage-item transform and uniform contract.
- [`field-scroll-driver.ts`](../../apps/web/features/field/scroll/field-scroll-driver.ts)
  is now a generic scroll controller fed by a surface-authored scroll manifest
  plus optional overlay adapters.
- [`field-scroll-state.ts`](../../apps/web/features/field/scroll/field-scroll-state.ts)
  resolves landing carry windows from manifest data instead of hardcoded
  section math.
- [`field-process-stage-controller.ts`](../../apps/web/features/field/surfaces/FieldLandingPage/field-process-stage-controller.ts)
...
The field should be one render layer inside a chapter system, not the
...
- use live Three.js for the field
...
The default field should look good before any composer pass exists.
...
- "build a hybrid chapter system, with the field as the continuous
  substrate"
...
does not help the field itself.
...
- no post stack in the field until the base field, shell, and overlays
...
- each visible field surface should feel like the same world
...
- the field should be derived from a published graph release
...
[`field-implementation.md`](./field-implementation.md) and the
...
- keep the field to one `Points` draw call in v1
...
[`field-implementation.md`](./field-implementation.md), but is
...
Do not ship any of these as the "field" system:
...
- [`visual-presets.ts`](../../apps/web/features/field/scene/visual-presets.ts)
- [`field-scroll-driver.ts`](../../apps/web/features/field/scroll/field-scroll-driver.ts)
- [`FieldLabPage.tsx`](../../apps/web/features/field/surfaces/FieldLabPage.tsx)
