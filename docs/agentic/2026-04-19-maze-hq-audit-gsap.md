...
**Scope**: BlobController, StreamController, PcbController, FieldController, ambient-field-scroll-driver  
...
| **ScrollTrigger.refresh() timing** | scripts.pretty.js: implicit (post-bind) | ambient-field-scroll-driver:106 (explicit call at line 106) | ✓ Aligned + explicit (better for React) | OK |
| **setTimeout(bindScroll, 1) deferral** | Not detected in Maze bindScroll path (async pattern used instead) | ambient-field-scroll-driver:41 (implicit, synchronous bind) | Difference: Maze defers; SoleMD binds sync. React lifecycle owns the defer. | OK for React |
...
- **Why**: SoleMD's React wrapper (ambient-field-scroll-driver) calls `bindScroll()` synchronously in a useEffect-like pattern, so the pause-on-init is implicit: the timeline is constructed *after* the DOM is ready, and ScrollTrigger's scrub control begins immediately.
...
- **SoleMD**: ambient-field-scroll-driver:29 ("Landing-only binder: the landing is a blob-centric story, so only the blob layer is wired here")
