...
**SoleMD source re-read**: `apps/web/features/field/scroll/field-scroll-driver.ts` (114 lines, last touched commit `6e75031` 2026-04-19 18:43 — post-pilot)
...
### D1 — Singleton class `jt` vs. function-based `bindFieldControllers`
...
- **SoleMD verification**: `field-scroll-driver.ts:34` exports `bindFieldControllers(...)` returning `() => void`. No singleton, no static lifecycle. Confirmed present as described.
...
- **Verdict**: **confirmed** as a true gap. However, the pilot itself flagged at line 124 that this was "re-verified" as a possible delegation — re-verified here that there is no delegation anywhere in `apps/web`; this is genuinely not implemented. The pilot's "Must-fix" severity **should be downgraded to "Should-fix / delegated-to-shell"** in the build spec: the capability doesn't exist, but per SKILL.md § 1 Stage layer, a shell-level utility (`bindScrollStateClasses`) is the correct owner — not the field scroll-driver. The pilot's proposed split module is correct.
...
- **Verdict**: **confirmed** as a deferred-gap. Correctly classified "Should-fix, delegated to router/shell." Not the field layer's job.
...
- **Verdict**: **confirmed** as gap. Correctly "Should-fix / delegated to shell utility." Pilot's proposed `field-dom-observers.ts` split is right.
...
- **SoleMD location**: `field-scroll-driver.ts:79–90`.
...
- **SoleMD location**: `field-scroll-driver.ts:106`, with a meaningful block comment (97–105) explaining why a synchronous refresh is required in React when multiple `fromTo` tweens stack on the same uniform (`uAlpha 1→floor` then `floor→1`).
...
- **SoleMD location**: `field-scroll-driver.ts:50, 91–95`. When `reducedMotion`, the driver pins `blob.visibility = 1` and `--ambient-hero-progress = 0`, skipping all ScrollTrigger construction.
...
- **SoleMD location**: `field-scroll-driver.ts:59–72`. `onUpdate` writes `localProgress` and `visibility` on `sceneStateRef.current.items.blob` — the bridge by which non-blob controllers could later observe blob progress.
