...
| CA-008 | completed | P1 | Doc parsing quality | Markdown/docs chunking audit found table splitting mid-row, missing test coverage for tables/inline-code/empty-code-blocks, and LOC growth in search handler | Added table-aware line splitting in `text_split.py`, extracted doc-enrichment (248 LOC) and type-flow (148 LOC) into separate handler modules, fixed test mock regression (4 tests), added skip-on-no-Postgres for 12 job tests, renamed colliding test file, added 5 new chunking tests, added debug logging to 5 swallowed exceptions | `1002 passed, 19 skipped`, chunking tests `74 passed`, search.py 906→541 LOC |
...
| CA-028 | completed | P1 | Nightly force-reindex scheduler | Index drift between full reindexes caused 0-chunk results for recently modified files | Added per-project nightly reindex worker to transport lifecycle with configurable UTC hour (`CODE_SEARCH_NIGHTLY_REINDEX_HOUR`, default 3), stable per-project jitter (0-15 min via SHA256 of project name) to stagger TEI/Qdrant load, and daily loop with error backoff | py-compile, transport/config tests `19 passed`, config field validated |
