# Ambient Field Maze Baseline Ledger — Round 12
...
`ambient-field-maze-baseline-ledger.md` (rounds 1–11). Round 12 treats the
ambient-field as the **foundation for all future SoleMD modules**
...
`docs/map/ambient-field-maze-baseline-ledger-assets/round-12-phase-*/`.

---

## Goal

Rebuild the ambient-field feature against Maze's homepage runtime as the
...
- Out of scope: `apps/worker/`, `db/`, `packages/`, `docs/rag/`, and
  any other dirty worktree file outside ambient-field.
- `AmbientFieldGraphWarmupAction.tsx` CSS custom-property typecheck issue:
...
### 18. Current ambient-field gaps (file:line, pre-round)

- `apps/web/features/ambient-field/renderer/field-shaders.ts:222-286` —
...
- `scroll/ambient-field-scroll-state.ts:143-181` — still correct as a
  target producer; keep and feed into scrubber.
- `surfaces/AmbientFieldLandingPage/AmbientFieldLandingPage.tsx:340-358` —
  static div for hotspot ring; no SVG circle + stroke-dashoffset
  primitive, no CSS keyframes matching Maze.
- `surfaces/AmbientFieldLandingPage/ambient-field-hotspot-overlay.ts:37,44,51` —
...
All primitives live under `apps/web/features/ambient-field/` and are
...
| `AmbientFieldHotspotRing` + lifecycle | `overlay/AmbientFieldHotspotRing.tsx`, `overlay/ambient-field-hotspot-ring.css`, `overlay/ambient-field-hotspot-lifecycle.ts` | Exact Maze CSS keyframes + per-hotspot reseed + phase-gate contract. |
...
| `UniformScrubber` | `scroll/ambient-field-uniform-scrubber.ts` | GSAP `scrub:1` emulation (1 s half-life low-pass). |
...
- Files touched:
  ...
  - `apps/web/features/ambient-field/renderer/FieldScene.tsx` — update
...
- Tests: 3 suites / 12 tests green
  (`point-source-registry`, `ambient-field-scroll-state`,
  `ambient-field-hotspot-overlay`).
- Typecheck: only the pre-existing `AmbientFieldGraphWarmupAction.tsx`
...
- Files created:
  ...
  - `apps/web/features/ambient-field/asset/__tests__/field-attribute-baker.test.ts`
  ...
  - `apps/web/features/ambient-field/asset/__tests__/field-geometry.test.ts`
...
- Files refactored:
  - `apps/web/features/ambient-field/asset/point-source-registry.ts` —
    ...
    `AMBIENT_FIELD_BUCKET_INDEX` for downstream consumers.
  - `apps/web/features/ambient-field/asset/point-source-types.ts` —
    added `aBucket: Float32Array` to `AmbientFieldPointSourceBuffers`.
  - `apps/web/features/ambient-field/asset/__tests__/point-source-registry.test.ts`
    — added 2 tests: `aBucket` range + integer, bucket histogram ±2%.
  - `apps/web/features/ambient-field/renderer/FieldScene.tsx` — binds
...
- Tests: 5 suites / 30 tests green (`field-attribute-baker`,
  `field-geometry`, `point-source-registry`, `ambient-field-scroll-state`,
  `ambient-field-hotspot-overlay`).
- Typecheck: no new failures. Pre-existing out-of-scope failures remain
  (`AmbientFieldGraphWarmupAction.tsx`, `PromptBoxSurface.tsx`).
...
- Files created:
  ...
  - `apps/web/features/ambient-field/asset/__tests__/image-point-source.test.ts`
    — 3 tests: red-channel default, layers multiplier, zero-threshold
    fallthrough.
  - `apps/web/features/ambient-field/asset/__tests__/model-point-source.test.ts`
...
- Typecheck: no new failures; out-of-scope `AmbientFieldGraphWarmupAction.tsx`
...
- Browser A/B capture (visible Chrome via CDP, 4 screenshots):
  ...
  All under
  `docs/map/ambient-field-maze-baseline-ledger-assets/round-12-phase-3/`.
...
- Files created:
  - `apps/web/features/ambient-field/renderer/field-loop-clock.ts` —
    module-scoped epoch, `getAmbientFieldElapsedMs` / `getAmbientFieldElapsedSeconds`,
    `__resetAmbientFieldLoopClockForTests`.
  - `apps/web/features/ambient-field/scroll/ambient-field-uniform-scrubber.ts` —
  ...
  - `apps/web/features/ambient-field/scroll/__tests__/ambient-field-uniform-scrubber.test.ts`
    — 7 tests (half-life, two half-lives, dt=0 no-op, first-step no-op
    default, full reset, partial reset, multi-key independence).
- Files refactored:
  - `apps/web/features/ambient-field/renderer/FieldScene.tsx` — swapped
    per-component `loopEpochMsRef` + inline `getAmbientFieldLoopSeconds`
    for the singleton clock.
- Tests: 8 suites / 45 tests green.
- Typecheck: clean (pre-existing `AmbientFieldGraphWarmupAction.tsx` only).
...
- Files created:
  - `apps/web/features/ambient-field/scene/burst-config.ts` —
  ...
  - `apps/web/features/ambient-field/renderer/burst-controller.ts` —
...
- Files modified:
  - `apps/web/features/ambient-field/renderer/field-shaders.ts` — added
...
- Scope note: the plan's full inline-hotspot refactor is deferred into
  ...
  base controller.
- Files created:
  - `apps/web/features/ambient-field/controller/FieldController.ts` —
...
- Files modified:
  - `apps/web/features/ambient-field/renderer/FieldScene.tsx` — adds a
...
- Remaining gaps: FieldScene still computes `uScale`/`wrapper.scale`
  ...
  the new `AmbientFieldHotspotRing` primitive lands.
...
- 9a: `apps/web/features/ambient-field/index.ts` rewritten as the public
  ...
  AmbientFieldHotspotRing + lifecycle, uniform scrubber, chapter timeline +
  chapters, landing surface. Downstream modules should import exclusively
  from `@/features/ambient-field`.

- 9b (executed via the `ambient-field-round-12-closeout` team + an earlier
  ...
  Nine final files under `.claude/skills/ambient-field-modules/references/`:
  ...
  - `maze-stage-overlay-contract.md` — 210 → 305 lines.
    `AmbientFieldHotspotRing` + `createHotspotLifecycleController` + the
...
- 9b Round 4 (SKILL.md updates across skills):
  - `ambient-field-modules/SKILL.md` bumped to 1.7.0; canonical-sources
    ...
    each enumerated).
  - `graph/references/frontend-performance.md` gained an "Ambient Field
  ...
  - Animation-authoring / learn-modules / cosmograph SKILLs already
    reference `/ambient-field-modules` as a companion; no additional
...
- Final ambient-field test pass: **10 suites / 56 tests green**
  (`field-attribute-baker`, `field-geometry`, `image-point-source`,
  `model-point-source`, `point-source-registry`, `ambient-field-scroll-state`,
  `ambient-field-uniform-scrubber`, `field-chapter-timeline`,
  `ambient-field-hotspot-lifecycle`, `ambient-field-hotspot-overlay`).
- Final `npm run typecheck`: only the pre-existing plan-acknowledged
  failure `AmbientFieldGraphWarmupAction.tsx(31,3) TS2353` remains.
...
- Remaining parity gaps (not regressions; explicit deferrals for a
  ...
  - FieldScene hotspot machinery remains inline (~300 lines). Phase 7's
    new `AmbientFieldHotspotRing` + lifecycle controller is the
    target primitive; the landing page hasn't been refactored onto it
    yet. The existing `resolveAmbientFieldFocusPresentation` flow still
...
- New code files: 21 under `apps/web/features/ambient-field/`.
