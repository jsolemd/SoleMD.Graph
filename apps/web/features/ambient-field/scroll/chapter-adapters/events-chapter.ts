...
function prepareCheckmarkPaths(element: HTMLElement) {
  return Array.from(
    element.querySelectorAll<SVGPathElement>("[data-event-checkmark-path]"),
  ).map((path) => {
...
export const eventsChapterAdapter: ChapterAdapter = (element, options) => {
  const main = element.querySelector<HTMLElement>("[data-events-main]");
  const items = Array.from(
    element.querySelectorAll<HTMLElement>("[data-event-subitem]"),
  );
  ...
  if (options.reducedMotion) {
    ...
    items.forEach((item) => {
      ...
      item
        .querySelectorAll<HTMLElement>("[data-event-number], [data-event-text]")
        .forEach((node) => {
  ...
  items.forEach((item, index) => {
    const number = item.querySelector<HTMLElement>("[data-event-number]");
    const text = item.querySelector<HTMLElement>("[data-event-text]");
    const path = item.querySelector<SVGPathElement>("[data-event-checkmark-path]");
