...
| M3 | One entry actor per release; file-level messages allowed only behind a durable DB task ledger and idempotent finalizer (05 §14.2). | Updated: `ingest.start_release` owns release state; S2 citations use `ingest.s2_citation_file` on `ingest_file` with `solemd.ingest_file_tasks`. | OK | actors/ingest.py, writers/s2_citations.py |
| M4 | Resume keyed to `families_loaded TEXT[]` for family completion; large families may add a narrower DB task ledger. | Updated: S2 citations now have `solemd.ingest_file_tasks` plus `s2_paper_reference_metrics_file_checkpoints`; old family-level-only finding is stale for citations. | OK | runtime.py, writers/s2_citation_tasks.py |
