...
- Warehouse `pg-data/` grows with the selected corpus, mapped enrichment,
  PubTator, grounding, and chunk lineage. Broad S2 citations are no longer a
  corpus-fill prerequisite and must not be the default warehouse-growth driver.
...
| S2 source-retention cleanup via `app.main source-retention`     | Hot raw files leave the VHD only after advisory-lock, manifest validation, and the S2 diff-cursor safety contract. Unconsumed mapped/evidence inputs are keep/cold-archive candidates, not automatic delete candidates. |
