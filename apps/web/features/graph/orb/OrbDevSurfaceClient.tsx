...
export function OrbDevSurfaceClient({
  bundle,
  fixturePath = null,
}: OrbDevSurfaceProps) {
  ...
  // Always enable so the hook's `canvas == null` branch produces synthetic
  // mock data when no bundle is available — keeps the sandbox renderable
  // even when the backend is cold or the bundle fetch returned null.
  ...
  // --- render --------------------------------------------------------
  // Render as soon as point-buffers have data (real or synthetic). We no
  // longer block on session readiness, since the hook produces synthetic
  // mock data when canvas is null.
  const showLoading =
    buffersState.status === "loading" || buffersState.data == null;
