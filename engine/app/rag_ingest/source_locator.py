...
WHERE
    ...
    OR source_locator.member_name IS NOT excluded.member_name
"""
...
def _ensure_optional_columns(conn: sqlite3.Connection) -> None:
    columns = {
        str(row["name"])
        for row in conn.execute("PRAGMA table_info(source_locator)").fetchall()
    }
    if "member_name" not in columns:
        conn.execute("ALTER TABLE source_locator ADD COLUMN member_name TEXT NULL")
