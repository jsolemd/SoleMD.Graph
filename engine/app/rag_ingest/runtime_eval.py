...
def _push_results_to_langfuse(results: Sequence[RuntimeEvalCaseResult]) -> None:
    """Legacy stub — score push is now handled by experiment.py via run_experiment().

    Kept as a no-op so callers of the ad-hoc evaluation path don't break.
    Use ``scripts/rag_benchmark.py`` for Langfuse-native evaluation.
    """
    pass
