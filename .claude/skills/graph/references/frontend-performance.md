...
## Field / Orb / Particle Runtime

For the field/orb/particle runtime perf contract (uTime continuity, scroll
scrubbers, chapter choreography, hotspot lifecycle, DPR caps, burst
controllers, mouse parallax limits), see `/module`. That skill owns the
shared fixed-stage WebGL substrate; `/graph` does not.

## References

- `../SKILL.md` for graph ownership and companion-skill routing
- `/module` for the field/orb/particle runtime contract
- `docs/map/field-maze-baseline-ledger-round-12.md` for the field Source
  Ground Truth + Foundation Primitives + Phase Log
