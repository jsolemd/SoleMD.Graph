...
if TYPE_CHECKING:
    from app.rag_ingest.write_sql_contract import StageSqlTemplateSpec
        ...
        for expected_order, stage in enumerate(self.stages, start=1):
            ...
            for dependency in stage.logical_dependencies:
                if dependency in planned_stage_set and dependency not in seen_stage_order:
                    raise ValueError(
                        f"{stage.stage} depends on {dependency}, "
                        "which must appear earlier in the plan"
                    )
...
def build_write_stage_specs() -> list[WriteStageSpec]:
    ...
    schema_table_names = {spec.table_name for spec in build_warehouse_table_specs()}
    for stage_spec in _WRITE_STAGE_SPECS:
        if stage_spec.table_name not in schema_table_names:
            raise ValueError(
                "write stage "
                f"{stage_spec.stage} references unknown schema table "
                f"{stage_spec.table_name}"
            )
...
_REFERENCE_ADAPTER_COPY_SQL = (
    f"COPY {_REFERENCE_ADAPTER_STAGING_TABLE} ({', '.join(_REFERENCE_ADAPTER_COLUMNS)}) FROM STDIN"
)
_REFERENCE_ADAPTER_DELETE_SQL = "DELETE FROM solemd.paper_references WHERE corpus_id = ANY(%s)"
...
class PostgresRagWriteRepository:
    ...
    def __init__(self, connect=None, table_exists_probe=None):
        ...
        from app.rag_ingest.write_sql_contract import build_write_sql_stage_spec_map
    ...
    def _replace_existing_rows_with_cursor(self, *, cur, batch: RagWarehouseWriteBatch) -> None:
        ...
        chunk_corpus_ids = sorted(
            {int(row.corpus_id) for row in [*batch.chunks, *batch.chunk_members]}
        )
        chunk_version_keys = sorted(
            {
                row.chunk_version_key
                for row in [*batch.chunk_versions, *batch.chunks, *batch.chunk_members]
                if row.chunk_version_key
            }
        )
        if not chunk_corpus_ids or not chunk_version_keys:
            return

        if self._table_exists_probe(cur, "solemd", "paper_chunk_members"):
            cur.execute(
                """
                ...
                """,
                (chunk_corpus_ids, chunk_version_keys),
            )
        if self._table_exists_probe(cur, "solemd", "paper_chunks"):
            cur.execute(
                """
