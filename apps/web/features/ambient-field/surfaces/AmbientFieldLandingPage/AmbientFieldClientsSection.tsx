...
import {
  ambientFieldClientItems,
  type AmbientFieldLandingSection,
} from "./ambient-field-landing-content";

interface AmbientFieldClientsSectionProps {
  section: AmbientFieldLandingSection;
}

export function AmbientFieldClientsSection({
  section,
}: AmbientFieldClientsSectionProps) {
  const sectionRef = useRef<HTMLElement | null>(null);
  useChapterAdapter(sectionRef, "clients");
    ...
    >
      <div className="mx-auto max-w-[1240px]">
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-2">
          {ambientFieldClientItems.map((item, index) => (
            <article
              key={item.id}
              data-clients-item
