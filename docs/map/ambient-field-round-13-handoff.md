# Ambient Field — Round 13 Handoff
...
This is the handoff for the next agent picking up the ambient-field work.
...
2. Then `/ambient-field-modules`. The skill's canonical-sources list
...
- `apps/web/features/ambient-field/renderer/FieldScene.tsx` L575-581
...
- Update `AmbientFieldHotspotRing` / overlay surfaces unaffected.

**Export surface to verify:**

- Keep `attachMouseParallax` as a public export in
  `apps/web/features/ambient-field/index.ts`. It already is (via
  `renderer/mouse-parallax-wrapper.ts` barrel re-export).
- In the `ambient-field-modules` skill, document explicitly:
...
1. `apps/web/features/ambient-field/renderer/FieldScene.tsx:575-581` —
   delete the useEffect.
2. `.claude/skills/ambient-field-modules/references/maze-stage-overlay-contract.md` —
   correct the Mouse Parallax Wrapper section so it's clear it applies
   per-module on opt-in, not globally.
3. `.claude/skills/ambient-field-modules/references/maze-mobile-performance-contract.md` —
...
`apps/web/features/ambient-field/scene/visual-presets.ts:139-170`).
...
`docs/map/ambient-field-maze-baseline-ledger-assets/round-12-phase-3/maze-scroll-0.png`
...
+ 0.3 under
`docs/map/ambient-field-maze-baseline-ledger-assets/round-13-phase-globe/`.
Paste into a new `docs/map/ambient-field-round-13-ledger.md` (create
...
`apps/web/features/ambient-field/scene/visual-presets.ts:115`. That's
...
1. `apps/web/features/ambient-field/scene/visual-presets.ts:115` —
...
**Current state:** `apps/web/features/ambient-field/scene/burst-config.ts`
...
- Expand `SOLEMD_BURST_COLORS` in
  `apps/web/features/ambient-field/scene/burst-config.ts` so every
...
- **Regular particles stay their current color** — do NOT touch the
  ...
  The Maze cyan→magenta binary-lerp is the ambient field; only the
...
1. `apps/web/features/ambient-field/scene/burst-config.ts` — expand
   ...
   diverge).
2. `apps/web/features/ambient-field/asset/field-attribute-baker.ts` —
   expand `SOLEMD_DEFAULT_BUCKETS`. Keep Maze-derived motion ranges.
3. `apps/web/features/ambient-field/asset/point-source-registry.ts` —
...
4. `apps/web/features/ambient-field/asset/__tests__/field-attribute-baker.test.ts` —
   update the histogram ±2 % assertions for the new bucket count +
   weights.
5. `apps/web/features/ambient-field/renderer/burst-controller.ts` —
   no change required in signature; color resolution already runs
   through `semanticColorMap[bucketId]`.
6. `.claude/skills/ambient-field-modules/references/maze-shader-material-contract.md`
...
- Visual pass: scrub through the landing page and confirm at least 3
  ...
  `docs/map/ambient-field-maze-baseline-ledger-assets/round-13-phase-bursts/`.

## Non-negotiables (do not regress Round 12)

These are invariants from Round 12 — a passing
`npx jest --testPathPatterns='features/ambient-field'` run + `npm run
typecheck` with no new errors are the gate.

1. **uTime never resets.** `getAmbientFieldElapsedSeconds()` is a
...
| File | Why |
|---|---|
| `docs/map/ambient-field-maze-baseline-ledger-round-12.md` | Source Ground Truth + Foundation Primitives table + Phase Log. |
| `.claude/skills/ambient-field-modules/SKILL.md` | Skill contract for any module that uses the shared stage. |
| `.claude/skills/ambient-field-modules/references/round-12-module-authoring.md` | Step-by-step authoring guide with 3 worked examples. |
| `.claude/skills/ambient-field-modules/references/maze-shader-material-contract.md` | Canonical shader + uniform shape, Maze source citations. |
| `.claude/skills/ambient-field-modules/references/maze-rebuild-checklist.md` | Quick DONE/OPEN audit by primitive. |
| `apps/web/features/ambient-field/index.ts` | Public barrel — every primitive exports through here. |
| `apps/web/features/ambient-field/scene/visual-presets.ts` | Blob/stream/pcb preset values (your scale/rotation/amplitude/frequency knobs). |
| `apps/web/features/ambient-field/scene/burst-config.ts` | Bucket id → color map. |
| `apps/web/features/ambient-field/asset/field-attribute-baker.ts` | `SOLEMD_DEFAULT_BUCKETS` (add new semantic entries here). |
| `apps/web/features/ambient-field/renderer/FieldScene.tsx:575-581` | Parallax useEffect to remove. |
| `apps/web/features/ambient-field/scroll/chapters/landing-blob-chapter.ts` | Event list that governs how the blob expands on scroll. |
...
Stay within `apps/web/features/ambient-field/`, the two skill
reference files called out above, and a new
`docs/map/ambient-field-round-13-*` ledger + asset dir. Do **not**
