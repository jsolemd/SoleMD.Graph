...
- `ambient-field-runtime.md` defines the product contract
- `ambient-field-implementation.md` defines the stack, dependency choices,
...
- landing process overlay geometry and sequencing:
...
This is the runtime future modules should extend.
...
- product term: `Ambient Field`
- feature package: `apps/web/features/ambient-field/`
...
for the actual feature directory. `ambient-field` is the more honest name for
...
- there is no shared shader or material abstraction today. A new
  `ambient-field` package should become that authority instead of trying to
...
- the ambient field should be one or a few draw calls, not a scene graph forest
...
- one dedicated internal `apps/web/app/ambient-field-lab/` route
...
- blob, stream, and pcb-like scenes can be inspected independently
...
### Phase 5. Rebuild pcb/cta parity and homepage shell composition
