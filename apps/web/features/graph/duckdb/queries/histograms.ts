...
export async function queryInfoHistogram(
  ...
  const result = await queryInfoHistogramsBatch(conn, {
    ...args,
    columns: [args.column],
    extentsByColumn: args.extent ? { [args.column]: args.extent } : undefined,
  })
  return result[args.column] ?? { bins: [], totalCount: 0 }
}
