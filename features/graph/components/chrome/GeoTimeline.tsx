...
import { safeMax } from "@/features/graph/lib/helpers";
...
export function GeoTimeline({ geoNodes }: GeoTimelineProps) {
  ...
  const { bars, minYear, maxYear } = useMemo(() => {
    // O(nodes + years) interval-counting approach instead of O(nodes * yearSpan)
    const events: Record<number, number> = {};
    for (const node of geoNodes) {
      if (node.firstYear != null) {
        const first = node.firstYear;
        const last = node.lastYear ?? first;
        events[first] = (events[first] ?? 0) + 1;
        events[last + 1] = (events[last + 1] ?? 0) - 1;
      }
    }
    const eventYears = Object.keys(events).map(Number).sort((a, b) => a - b);
    if (eventYears.length === 0) return { bars: [], minYear: 0, maxYear: 0 };

    // Prefix-sum sweep to build cumulative counts per year
    const buckets = new Map<number, number>();
    const minY = eventYears[0];
    // The last event year is lastYear+1, so actual data ends one before
    const maxY = eventYears[eventYears.length - 1] - 1;
    let running = 0;
    for (let y = minY; y <= maxY; y++) {
      running += events[y] ?? 0;
      if (running > 0) buckets.set(y, running);
    }

    const years = [...buckets.keys()].sort((a, b) => a - b);
    if (years.length === 0) return { bars: [], minYear: 0, maxYear: 0 };

    const maxCount = safeMax([...buckets.values()]);
  ...
  return (
    ...
    >
      <div className="relative h-full w-full">
        {/* Histogram bars */}
        <div className="absolute inset-0 flex items-end px-1" role="img" aria-label={`Year distribution from ${minYear} to ${maxYear}`}>
