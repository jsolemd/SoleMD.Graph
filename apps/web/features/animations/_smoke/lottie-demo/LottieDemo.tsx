"use client";
/**
 * D10 smoke test — cute pastel brain.
 *
 * Path geometry adapted from Twemoji (Twitter's open-source emoji set,
 * MIT licensed) brain glyph 1f9e0. The shape and gyri pattern are
 * unchanged from the original — only the fill colors have been remapped
 * to the SoleMD brand palette, and the whole thing is wrapped in a
 * Framer Motion `<motion.g>` for a gentle 2.4s breath.
 *
 * Why not Lottie: the first attempt at this component was a hand-
 * authored Lottie, which is the wrong medium for organic shapes with
 * many small features — the bezier-tangent mental model doesn't scale
 * past a few dozen control points. SVG path commands do. The Lottie
 * playback path remains validated by earlier work; a real LottieFiles
 * asset can slot in later without changing the pipeline.
 *
...
// Brand palette doesn't include a dark-pink accent, so we derive one
// from --color-soft-pink via color-mix(). Results stay on-brand
// because the base color is still a brand token — we're just shifting
// lightness for the gyri and shadow layers.
const FILL_BODY = "var(--color-soft-pink)";                                             // was #F4ABBA
const FILL_SHADOW = "color-mix(in srgb, var(--color-soft-pink) 58%, #2f0e1b 42%)";      // was #EA596E
const FILL_SHADOW_DEEP = "color-mix(in srgb, var(--color-soft-pink) 38%, #230913 62%)"; // was #DD2E44
  ...
  return (
    ...
    >
      <motion.svg
        ...
        role="img"
        aria-label="Cute pastel brain (Twemoji variant)"
      ...
      >
        ...
        />
        {/* Gyri — dense inner detail drawn as filled shapes, not stroked */}
        <g fill={FILL_SHADOW} fillOpacity={0.85}>
