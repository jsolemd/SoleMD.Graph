...
from app.rag.chunk_grounding import build_grounded_answer_from_chunks


def test_build_grounded_answer_from_chunks_builds_packets_and_answer_links():
    conn = MagicMock()
    cur = MagicMock()
    ...
    grounded = build_grounded_answer_from_chunks(
        corpus_ids=[12345],
...
def test_build_grounded_answer_from_chunks_returns_none_without_citation_packets():
    conn = MagicMock()
    cur = MagicMock()
    conn.__enter__.return_value = conn
    conn.__exit__.return_value = False
...
def test_build_grounded_answer_from_chunks_falls_back_to_entity_only_packets():
    conn = MagicMock()
    cur = MagicMock()
    conn.__enter__.return_value = conn
    conn.__exit__.return_value = False
