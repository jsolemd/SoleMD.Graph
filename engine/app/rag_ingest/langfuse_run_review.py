...
def review_experiment_result(
    ...
    *,
    ...
    by_family_inputs: dict[str, dict[str, list[float] | Counter[str]]] = {}
    ...
    for item_result in getattr(result, "item_results", []) or []:
        ...
        family_bucket = by_family_inputs.setdefault(
            family,
            {
                ...
                "routing_match": [],
                "duration_ms": [],
                "retrieval_profile_counts": Counter(),
                "warehouse_depth_counts": Counter(),
                "route_signature_counts": Counter(),
        ...
        family_bucket["hit_at_1"].append(hit_at_1)
        family_bucket["hit_at_k"].append(hit_at_k)
        family_bucket["grounded_answer_rate"].append(grounded)
        family_bucket["target_in_answer_corpus"].append(target_in_answer)
        family_bucket["duration_ms"].append(duration_ms)
        if routing_match is not None:
            family_bucket["routing_match"].append(float(routing_match))

        retrieval_profile = str(output.get("retrieval_profile") or "unknown")
        warehouse_depth = str(output.get("warehouse_depth") or "unknown")
        route_signature = str(output.get("route_signature") or "unknown")
        family_bucket["retrieval_profile_counts"][retrieval_profile] += 1
        family_bucket["warehouse_depth_counts"][warehouse_depth] += 1
        family_bucket["route_signature_counts"][route_signature] += 1
    ...
    for family, bucket in sorted(by_family_inputs.items()):
        ...
        by_family[family] = {
            ...
            "retrieval_profile_counts": dict(
                sorted(bucket["retrieval_profile_counts"].items())
            ),
            "warehouse_depth_counts": dict(
                sorted(bucket["warehouse_depth_counts"].items())
            ),
            "route_signature_counts": dict(
                bucket["route_signature_counts"].most_common(5)
            ),
            "miss_category_counts": dict(
                sorted(bucket["miss_category_counts"].items())
            ),
        }
...
def format_experiment_review(review: dict[str, Any]) -> str:
    ...
    for family, family_review in review.get("by_family", {}).items():
        ...
        routing_text = (
            f"{routing_match:.3f}"
            if isinstance(routing_match, (int, float))
            else "n/a"
        )
        lines.append(
        ...
        lines.append(
            f"  retrieval_profiles={family_review['retrieval_profile_counts']}"
        )
        lines.append(
            f"  warehouse_depth={family_review['warehouse_depth_counts']}"
        )
        if family_review["miss_category_counts"]:
            lines.append(
                f"  miss_categories={family_review['miss_category_counts']}"
            )
