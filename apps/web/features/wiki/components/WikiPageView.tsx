...
export interface WikiPageGraphActions {
  onPaperClick: (graphPaperRef: string) => void;
  showPageOnGraph: () => Promise<void>;
  canShowPageOnGraph: boolean;
}

export interface WikiPageGraphActionProviderProps {
  children: (actions: WikiPageGraphActions) => ReactNode;
  currentSlug: string;
  pageGraphRefs: readonly string[];
  queries: GraphBundleQueries;
}

export type WikiPageGraphActionProvider = (
  props: WikiPageGraphActionProviderProps,
) => ReactNode;
...
interface WikiPageViewProps {
  ...
  graphActionProvider?: WikiPageGraphActionProvider;
...
const inertGraphActions: WikiPageGraphActions = {
  onPaperClick: () => {},
  showPageOnGraph: async () => {},
...
export function WikiPageView({
  ...
  queries,
  onNavigate,
  graphActionProvider: GraphActionProvider,
  tocAnchorRef,
}: WikiPageViewProps) {
  ...
  if (GraphActionProvider) {
    return (
      <GraphActionProvider
        currentSlug={slug}
        pageGraphRefs={pageGraphRefs}
        queries={queries}
      >
        {renderPage}
      </GraphActionProvider>
  ...
  return renderPage(inertGraphActions);
}
